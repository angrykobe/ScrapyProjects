...
class ProxyMiddleware(object):
    def __init__(self):
        ...
        try:
            with open('proxy.json', 'rb') as f:
                c = json.loads(f.read())
                for i in c:
                    self._PROXIES.append("{0}:{1}".format(i['ip'], i['port']))
        except:
            pass
    ...
    def process_request(self, request, spider):
        ...
        spider.logger.info("ProxyMiddleware is %s", proxy)
...
class RotateCookieMiddleware(object):
    def process_request(self, request, spider):
        if hasattr(spider, 'cookies') and spider.cookies:
            jar = random.choice(spider.cookies)
            if not request._meta.get('cookiejar'):
                request._meta['cookiejar'] = jar


class MaimaispiderSpiderMiddleware(object):
